...
class Entity(BaseModel, ABC):
    ...
    alternativeLabels: Optional[List[str]] = Field(None, description="Alternative name(s) for the Entity.", ordered=False)
    extensions: Optional[List[Extension]] = Field(None, description="A list of extensions to the Entity, that allow for capture of information not directly supported by elements defined in the model.", ordered=False)
...
class DomainEntity(Entity, ABC):
    ...
    mappings: Optional[List[ConceptMapping]] = Field(None, description="A list of mappings to concepts in terminologies or code systems. Each mapping should include a coding and a relation.", ordered=False)
...
class Agent(Entity):
    ...
    type: Literal["Agent"] = Field(CoreImType.AGENT.value, description=f"MUST be '{CoreImType.AGENT.value}'.", extends="type")
...
class Activity(ActivityBase):
    ...
    performedBy: Optional[List[Agent] ]= Field(None, description="An Agent who contributed to executing the Activity.", ordered=False)
...
class Contribution(ActivityBase):
    ...
    type: Literal["Contribution"] = Field(CoreImType.CONTRIBUTION.value, description=f"MUST be '{CoreImType.CONTRIBUTION.value}'.", extends="type")
    contributor: Optional[List[Agent]] = Field(None, description="The agent that made the contribution.", min_length=1, max_length=1, extends="performedBy")
...
class InformationEntityBase(Entity, ABC):
    ...
    type: Literal["InformationEntity"] = Field(CoreImType.INFORMATION_ENTITY.value, description=f"MUST be {CoreImType.INFORMATION_ENTITY.value}.", extends="type")
    specifiedBy: Optional[Union[Method, IRI]] = Field(None, description="A specification that describes all or part of the process that led to creation of the Information Entity", ordered=False)
    contributions: Optional[List[Contribution] ]= Field(None, description="Specific actions taken by an Agent toward the creation, modification, validation, or deprecation of an Information Entity.", ordered=False)
    reportedIn: Optional[List[Union[Document, IRI]]] = Field(None, description="A document in which the the Information Entity is reported.", ordered=False)
...
class InformationEntity(InformationEntityBase):
    ...
    derivedFrom: Optional[List[InformationEntity]] = Field(None, description="Another Information Entity from which this Information Entity is derived, in whole or in part.", ordered=False)
...
class Document(InformationEntity):
    ...
    type: Literal["Document"] = Field(CoreImType.DOCUMENT.value, description=f"Must be '{CoreImType.DOCUMENT.value}'", extends="type")
    ...
    urls: Optional[List[Annotated[str, StringConstraints(pattern=r"^(https?|s?ftp)://")]]] = Field(
        None, description="One or more URLs from which the content of the Document can be retrieved.", ordered=False
    )
...
class Method(InformationEntity):
    ...
    type: Literal["Method"] = Field(CoreImType.METHOD.value, description=f"MUST be '{CoreImType.METHOD.value}'.", extends="type")
    subtype: Optional[Coding] = Field(
        None,
...
class RecordMetadata(BaseModel):
    ...
    derivedFrom: Optional[str] = Field(None, description="Another data record from which the record described here was derived, through a data ingest and/or transformation process. Value should be a string representing the identifier of the source record.", ordered=False)
    dateRecordCreated: Optional[str] = Field(None, description="The date the record was initially created.")
    contributions: Optional[List[Contribution]] = Field(None, description="Describes specific contributions made by an human or software agent to the creation, modification, or administrative management of a data record or object.", ordered=False)
...
class DataSet(InformationEntity):
    ...
    type: Literal["DataSet"] = Field(CoreImType.DATA_SET.value, description=f"MUST be '{CoreImType.DATA_SET.value}'.", extends="type")
...
class EvidenceLine(InformationEntity):
    ...
    type: Literal["EvidenceLine"] = Field(CoreImType.EVIDENCE_LINE.value, description=f"Must be '{CoreImType.EVIDENCE_LINE.value}'", extends="type")
    hasEvidenceItems: Optional[List[InformationEntity]] = Field(None, description="An individual piece of information that was evaluated as evidence in building the argument represented by an Evidence Line.", ordered=False)
...
class StatementBase(InformationEntity, ABC):
    ...
    hasEvidenceLines: Optional[List[EvidenceLine]] = Field(None, description="An evidence-based argument that supports or disputes the validity of the proposition that a Statement assesses or puts forth as true. The strength and direction of this argument (whether it supports or disputes the proposition, and how strongly) is based on an interpretation of one or more pieces of information as evidence (i.e. 'Evidence Items).", ordered=False)
...
class StudyGroup(Entity):
    ...
    isSubsetOf: Optional[List[StudyGroup] ]= Field(None, description="A larger StudyGroup of which this StudyGroup represents a subset.", ordered=False)
    characteristics: Optional[List[Characteristic]] = Field(None, description="A feature or role shared by all members of the StudyGroup, representing a criterion for membership in the group.", ordered=False)
...
class StudyResult(InformationEntityBase, ABC):
    ...
    sourceDataSet: Optional[List[DataSet]] = Field(None, description="A larger DataSet from which the content of the StudyResult was derived.", max_length=1)
    componentResult: Optional[List[StudyResult]] = Field(None, description="Another StudyResult comprised of data items about the same focus as its parent Result, but based on a more narrowly scoped analysis of the foundational data (e.g. an analysis based on data about a subset of the parent Results full study population) .", ordered=False)
